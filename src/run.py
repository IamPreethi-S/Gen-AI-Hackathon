...
with tab1:
    ...
    if st.button("Generate Lead Strategy", type="primary"):
        ...
        # Lead Qualification Framework
                        ...
                        with qual_col1:
                            st.markdown("**MQL Criteria**")
                            qualification = strategy.get('lead_generation_strategy', {}).get('qualification_framework', {})
                            mql_criteria = qualification.get('mql_criteria', [
                                "Company size matches target",
                                "Budget aligns with solution",
                                "Industry fit confirmed",
                                "Initial interest shown"
                            ])
                            for criteria in mql_criteria:
                                st.write(f"✓ {criteria}")

                        with qual_col2:
                            st.markdown("**SQL Criteria**")
                            sql_criteria = qualification.get('sql_criteria', [
                                "Decision maker engaged",
                                "Budget confirmed",
                                "Timeline established",
                                "Needs assessment completed"
                            ])
                            for criteria in sql_criteria:
                                st.write(f"✓ {criteria}")
            
                            # Score card
                            st.metric("Lead Quality Score", f"{qualification.get('score', 75)}/100")
                        
                    with strat_tab2:
        # Channel Strategy
                        st.subheader("Channel Strategy")
                        channel_strategy = strategy.get('lead_generation_strategy', {}).get('channel_strategy', {})
                        
                        # Default channels if none provided
                        channels = channel_strategy.get('primary_channels', [
                            {"channel": "LinkedIn", "allocation": 30, "focus": "B2B outreach"},
                            {"channel": "Email", "allocation": 25, "focus": "Nurture sequences"},
                            {"channel": "Content Marketing", "allocation": 25, "focus": "Thought leadership"},
                            {"channel": "Events", "allocation": 20, "focus": "Industry networking"}
                        ])
                        ...
                        for channel in channels:
                            ...
                            with col1:
                                st.progress(channel.get('allocation', 0)/100)
                            with col2:
                                st.write(f"{channel.get('allocation')}%")
                            st.markdown(f"**{channel.get('channel')}**: {channel.get('focus')}")
                            st.markdown("---")
    
                    with strat_tab3:
        # Content Strategy
                        st.subheader("Content Strategy")
                        content_strategy = strategy.get('lead_generation_strategy', {}).get('content_strategy', {})
                        
                        # Define content stages
                        stages = {
                            "Awareness": content_strategy.get('awareness_stage', [
                                "Industry trend reports",
                                "Educational webinars",
                                "Blog posts",
                                "Social media content"
                            ]),
                            "Consideration": content_strategy.get('consideration_stage', [
                                "Case studies",
                                "Product comparisons",
                                "Feature highlights",
                                "Demo videos"
                            ]),
                            "Decision": content_strategy.get('decision_stage', [
                                "ROI calculators",
                        ...
                        # Display content by stage
                        for stage, content_items in stages.items():
        ...
        # Outreach Strategy
                        st.subheader("Outreach Templates")
                        outreach_templates = strategy.get('lead_generation_strategy', {}).get('outreach_templates', {})
                        
                        # Email Templates
                        st.markdown("**📧 Email Sequences**")
                        email_templates = outreach_templates.get('email_templates', {
                            "Initial Contact": "Hi {first_name},\n\nI noticed {company} is leading in {industry}...",
                            "Follow-up": "Following up on my previous message about...",
                            "Meeting Request": "Would you be open to a 15-minute call to discuss..."
                        })
                        
                        for template_name, content in email_templates.items():
                            st.text_area(
                                f"{template_name} Template",
                                value=content,
                                height=100,
                                key=f"email_{template_name}"
                            )
                            st.markdown("---")
                        
                        # LinkedIn Templates
                        st.markdown("**🔗 LinkedIn Messages**")
                        linkedin_templates = outreach_templates.get('linkedin_messages', {
                            "Connection Request": "Hi {first_name}, I noticed your work in {industry}...",
                            "Follow-up": "Thanks for connecting! I thought you might be interested in..."
                        })
                        
                        for template_name, content in linkedin_templates.items():
                            st.text_area(
                                f"{template_name} Template",
                                value=content,
                                height=100,
                                key=f"linkedin_{template_name}"
                            )
                        
                        # Outreach Tips
                        st.markdown("**💡 Best Practices**")
                        tips = outreach_templates.get('best_practices', [
                            "Personalize each message with company-specific insights",
    ...
    # Store in session state with enhanced data
                lead_entry = {
                    ...
                    'status': 'New Strategy',
                    'score': qualification.get('score', 75),
                    'analysis': {
                        'qualification_framework': qualification,
